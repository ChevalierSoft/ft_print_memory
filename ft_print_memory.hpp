...
/*   Updated: 2022/05/14 03:06:15 by dait-atm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Displays a hexdump of memory chunk on stderr. (ported from C)
 * 
 * @param addr A pointer on the data you want to display
 * @param size How many bytes to display
 */
...
static
void	ft_prepare_oct_print(std::string &mem, const void *addr)
{
	const unsigned char	*l;

	l = reinterpret_cast<const unsigned char *>(addr);
	if ((*l & 0xff) == 0)
	{
		mem += FTPM_BRED;
...
static
int		ft_aff_oct(std::string &mem, const void *addr, unsigned int size)
{
	int		cpt;
	int		flagou;
	int		sp;
...
static
void	ft_aff_msg(std::string &mem, const void *addr, int nbr)
{
	char	*c;
